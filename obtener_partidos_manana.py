...
temporada = manana.year
fecha_busqueda = manana.strftime('%b %-d')  # ej: "Jun 15"

print(f"Buscando partidos para: {manana.date()} (formato BR: '{fecha_busqueda}')")
...
print(f"\nPartidos encontrados para {manana.date()}:")
for p in partidos:
    print(f"  {p['Visitante']} @ {p['Local']}  ({p['Fecha']})")

pd.DataFrame(partidos).to_csv('partidos_manana.csv', index=False)
print("\nGuardado en partidos_manana.csv")
